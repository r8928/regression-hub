...
function buildListQuery(teamId, filters) {
  ...
  if (filters.testedBy === '__unassigned__') {
  ...
  if (filters.version) query.softwareVersionTested = { $regex: filters.version, $options: 'i' };
  if (filters.priority) query.priority = filters.priority;
  if (filters.jiraStory) query.jiraStory = { $regex: filters.jiraStory, $options: 'i' };
  if (filters.status === 'Pass') query.status = 'Pass';
  else if (filters.status === 'Fail') query.status = 'Fail';
  else if (filters.status === 'Pending') query.status = { $nin: ['Pass', 'Fail'] };
  return query;
}
...
export async function resetTeamData(db, teamId) {
  ...
  const [testCases, testRuns, modules, applications, assignments] = await Promise.all([
