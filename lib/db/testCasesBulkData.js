...
function buildBulkMatchQuery(teamId, { ids, filter, pendingOnly }) {
  if (ids?.length) {
    const matchQuery = { _id: { $in: ids.map((id) => new ObjectId(id)) }, teamId };
  ...
  if (filter?.version) {
    matchQuery.softwareVersionTested = { $regex: filter.version, $options: 'i' };
  }
...
export async function bulkUpdateTestCases(db, teamId, { ids, filter, fields, pendingOnly }) {
