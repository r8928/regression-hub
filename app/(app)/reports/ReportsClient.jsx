...
import { drawPdfPageHeader, loadPdf } from "@/utils/pdfHelpers";
  ...
  async function exportExcel(overrideVersion) {
    ...
    try {
      ...
      const summaryRows = [
        ...
        [
          "Passed",
          cases.filter((t) => normalizedStatus(t.status) === "Pass").length,
        ],
        [
          "Failed",
          cases.filter((t) => normalizedStatus(t.status) === "Fail").length,
        ],
        [
          "Pending",
          cases.filter((t) => normalizedStatus(t.status) === "Pending").length,
        ],
  ...
  async function exportPdf(overrideVersion) {
    ...
    try {
      ...
      const passed = cases.filter(
        (t) => normalizedStatus(t.status) === "Pass"
      ).length;
      const failed = cases.filter(
        (t) => normalizedStatus(t.status) === "Fail"
      ).length;
      ...
      function drawDonutSegment(
        ...
        for (let i = 0; i <= steps; i++) {
          const a =
            ((startDeg + ((endDeg - startDeg) * i) / steps - 90) * Math.PI) /
            180;
      ...
      doc.text(
        `${appName}  ·  ${environment}  ·  v${version || "N/A"}`,
        ML,
        66
      );
      ...
      wfLabels.forEach((lbl, i) => {
        ...
        doc.text(lbl, ML + colW * i + colW / 2, wfTop + 24, {
          align: "center",
        });
      });
      ...
      doc.setFontSize(8);
      const wfVals = [
        `SW-RPT-Regression-${version || "v0"}`,
        "Regression Signoff Report",
        version || "N/A",
      ];
      wfVals.forEach((val, i) => {
        ...
        doc.text(val, ML + colW * i + colW / 2, wfTop + 41, {
          align: "center",
        });
      });
      ...
      for (const aName of appGroupNames) {
        ...
        const aPassed = appCases.filter(
          (t) => normalizedStatus(t.status) === "Pass"
        ).length;
        const aFailed = appCases.filter(
          (t) => normalizedStatus(t.status) === "Fail"
        ).length;
        ...
        doc.setFontSize(9);
        doc.setFont("helvetica", "normal");
        doc.text(`${aTotal} cases  ·  ${aPassPct}% pass rate`, W - MR, 22, {
          align: "right",
        });
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(14);
        doc.text(`${aName} — Regression Testing`, W / 2, 58, {
          align: "center",
        });
        ...
        doc.setFontSize(10);
        doc.setTextColor(40, 40, 40);
        doc.text(`Total Test Cases: ${aTotal}`, cx, statsY, {
          align: "center",
        });
        doc.text(`Total Passed: ${aPassed}`, cx, statsY + 14, {
          align: "center",
        });
        doc.text(`Total Failed: ${aFailed}`, cx, statsY + 28, {
          align: "center",
        });
        if (aPending > 0)
          doc.text(`Total Pending: ${aPending}`, cx, statsY + 42, {
            align: "center",
          });
