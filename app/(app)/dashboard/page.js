...
import { getDashboardData, getDashboardSettings } from '@/lib/dashboardData';
      ...
      <PageHeader
        ...
        actions={softwareVersion ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface-2)', border: '1px solid var(--line)', borderRadius: 8, padding: '8px 14px', fontSize: 13 }}>
            ...
            <span className='font-mono' style={{ background: 'rgba(13,148,136,0.12)', border: '1px solid rgba(13,148,136,0.4)', borderRadius: 6, padding: '2px 10px', fontWeight: 700, color: '#0d9488', fontSize: 13 }}>{softwareVersion}</span>
          </div>
        ) : undefined}
      />
      ...
      <div className='grid-2' style={{ marginBottom: 20 }}>
        ...
        <div className='panel'>
          ...
          {Object.keys(testerGroups).length ? (
            <div>
              {Object.entries(testerGroups).sort(([, a], [, b]) => b.total - a.total).map(([name, g]) => (
