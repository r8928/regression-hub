  ...
  async function toggleActive(u) {
    ...
    try {
      const res = await fetch(`/api/users/${u._id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ active: u.active === false }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
                          ...
                          {isEditing ? (
                            ...
                            <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                              <button
                                ...
                                onClick={() => { setEditId(u._id); setEditForm({ name: u.name, role: u.role }); }}
                                title='Edit name / role'
                              >Edit</button>
                              <button
                                className='btn btn-secondary btn-sm'
                                onClick={() => { setPwdId(u._id); setPwdForm(EMPTY_PWD); }}
        ...
        <Modal title='Add New User' onClose={() => { setShowAdd(false); setAddForm(EMPTY_FORM); }}>
          <form onSubmit={createUser} style={{ display: 'grid', gap: 14 }}>
            ...
            <div style={{ padding: '10px 14px', background: 'rgba(13,148,136,0.06)', borderRadius: 8, border: '1px solid rgba(13,148,136,0.2)', fontSize: 12, color: 'var(--muted)' }}>
              This user will be added to the <strong style={{ color: 'var(--fg)' }}>{user.teamName}</strong> location and can only see that location&apos;s data.
            </div>
